...
SECRET_KEY = os.getenv("SECRET_KEY") or 'Ludibrium-Secret-133.221.333.123.111_kvYAtbZkwkhyPv5B'
...
async def debug_user(request):
    if not request.session.get('debug', False):
        return TemplateResponse('show.html', {'request': request, 'note': request.session.get('debug', False)})
