...
def strings_init():
    ...
    for instr_addr, string_addr, string_len in retry:
        if create_string(string_addr, string_len):
            ...
            error('Unable to make a string @ 0x%x with length of %d for usage in function @ 0x%x' %
...
def create_runtime_ms():
    ...
    runtime_ms_end = idaapi.find_binary(
        text_seg.startEA, text_seg.endEA, opcodes, 0, SEARCH_DOWN)
...
def renamer_init():
    ...
    if gopclntab is not None:
        ...
        while addr < early_end:
            ...
            func_name_addr = Dword(
                name_offset + gopclntab.startEA + addr_size) + gopclntab.startEA
